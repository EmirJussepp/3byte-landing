      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        {pasos.map((p, i) => (
          <FadeIn
            key={i}
            delay={i * 0.08}
            className="rounded-none border border-white/[0.07] bg-[#0d0d11] p-6 flex flex-col"
          >
            {/* Número */}
            <div className="mb-5">
              <div className="w-9 h-9 rounded-none border border-[#5b8bff]/30 bg-[#5b8bff]/[0.06] flex items-center justify-center">
                <span className="font-mono text-[0.62rem] text-[#5b8bff] font-bold tracking-wider">{p.n}</span>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-white/[0.06]">
              <span className="font-mono text-[0.62rem] text-[#5b8bff] tracking-wide">
                → {p.detail}
              </span>
            </div>
          </FadeIn>
