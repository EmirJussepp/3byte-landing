...
const inputBase =
  "w-full bg-[#13131a] border text-[#eaeaf0] text-[0.92rem] placeholder:text-[#3a3a4a] outline-none transition-all duration-200 rounded-none px-4 py-3";
...
export default function ContactForm() {
  ...
  if (status === "ok") {
    return (
      <div className="flex flex-col items-start gap-4 py-10">
        <div className="w-11 h-11 rounded-none bg-[#34d399]/10 border border-[#34d399]/20 flex items-center justify-center">
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
      ...
      {/* Error general */}
      {status === "error" && (
        <p className="text-[0.85rem] text-[#f87171] bg-[#f87171]/5 border border-[#f87171]/20 rounded-none px-4 py-3">
          {errorMsg}
        </p>
      )}
      ...
      <div className="pt-1">
        <button
          ...
          className="inline-flex items-center gap-2.5 px-7 py-3.5 rounded-none bg-[#5b8bff] text-white font-bold text-[0.82rem] tracking-[0.04em] hover:bg-[#4a7aee] hover:-translate-y-px transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
        >
          {status === "loading" ? (
            <>
